...
1. **フィルタリング方式の切り替え**
    - 「📅 日時でフィルタリング」と「🎮 直近試合でフィルタリング」をラジオボタンで選択
    - デフォルトは日時フィルター（開始/終了日時＋クイックプリセットが表示）
    - 直近試合フィルターを選ぶと、試合数入力ボックスと「±1」ボタンのみが表示され、不要な UI を隠せます
2. **日時フィルター**
    - **開始日時/終了日時**: タイムゾーン対応の `datetime-local` で細かく指定
    - **クイックプリセット**: 直近 3/6/12/24 時間、2〜7 日、30 日をワンクリック適用
3. **直近試合フィルター**
    - 数値入力で「直近 ◯ 試合」を柔軟に指定（最小 1 試合）
    - 「+1」「-1」ボタンで素早く調整、Enter キーでも適用可能
4. **リセット**
    - どちらのモードでもボタンひとつで初期状態（直近 24 時間＆直近 20 試合）に戻せます

**適用範囲**: 概要・プレイヤー統計・ロール分析・ゲーム一覧のすべてが同じフィルタ結果を参照します。
...
-   ✅ **日時 / 直近試合の切り替えスイッチ**を追加し、必要な UI だけを表示
-   ✅ **直近試合フィルター**を入力ボックス＋「±1」ボタン＋ Enter 対応に刷新
-   ✅ **フィルター結果の適用範囲を統一**し、ロール分析・概要・ゲーム一覧も完全同期
-   ✅ **クイックプリセット**の挙動を整理し、フィルターモード毎に独立した操作感を実現
-   🔧 フィルター状態リセット時に日時/試合数のデフォルト値を自動復元

### v1.4 変更内容（UI レイアウト整理）
