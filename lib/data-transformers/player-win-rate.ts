import type { PlayerWinRateData, PlayerWinRateRow, TransformerOptions } from "./types";
import { applyCommonFilters, buildPlayerAggregates } from "./utils";

export function buildPlayerWinRateData(options: TransformerOptions): PlayerWinRateData {
  const games = applyCommonFilters(options);
  const aggregates = buildPlayerAggregates(games, options.selectedPlayerIds);

  const rows: PlayerWinRateRow[] = Array.from(aggregates.values()).map((aggregate) => {
    ...
    return {
      ...
      games: totalGames,
      winRate,
      factions: Object.entries(aggregate.factions).map(([faction, stats]) => ({
