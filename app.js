...
async function loadData() {
    try {
        ...
        document.body.innerHTML = '<div class="loading">データの読み込みに失敗しました</div>';
    }
}
...
function updateSummaryStats() {
    ...
    document.getElementById('totalPlayers').textContent = Object.keys(playerStats).length;
    document.getElementById('totalRoles').textContent = Object.keys(roleStats).length;
    ...
    const avgMinutes = filteredGames.length > 0 ? (totalMinutes / filteredGames.length).toFixed(1) : 0;
...
function updateWinnerTeamChart() {
    ...
    ctx.onclick = function(evt) {
        const points = charts.winnerTeam.getElementsAtEventForMode(evt, 'nearest', { intersect: true }, true);
...
function updatePlayerStatsTable() {
    ...
    const players = Object.entries(playerStats)
        .map(([name, stats]) => ({
            name,
            winRate: (stats.wins / stats.games * 100).toFixed(1),
            taskRate: stats.tasksTotal > 0 ? (stats.tasksCompleted / stats.tasksTotal * 100).toFixed(1) : 0,
            wins: stats.wins,
            games: stats.games
        }))
        .sort((a, b) => b.winRate - a.winRate);
...
function updateHeatmap() {
    ...
    const roles = [...new Set(Object.values(playerRoleMatrix).flatMap(r => Object.keys(r)))].sort();
    ...
    if (currentSortedRole) {
        players = players.sort((a, b) => {
            const aData = playerRoleMatrix[a][currentSortedRole] || { count: 0, wins: 0 };
            const bData = playerRoleMatrix[b][currentSortedRole] || { count: 0, wins: 0 };
    ...
    // 最大値を取得
    const maxValue = Math.max(...Object.values(playerRoleMatrix).flatMap(r => 
    ...
    roles.forEach(role => {
        ...
        const sortIcon = currentSortedRole === role 
    ...
    players.forEach((player, playerIndex) => {
        ...
        roles.forEach((role, roleIndex) => {
            const data = (playerRoleMatrix[player] && playerRoleMatrix[player][role]) || { count: 0, wins: 0 };
...
function sortHeatmapByRole(role) {
    ...
    anime({
        ...
        complete: function() {
            ...
            // 新しい行を取得してアニメーション
            setTimeout(() => {
                const newRows = container.querySelector('tbody')?.querySelectorAll('tr');
                if (newRows) {
                    // 右からスライドイン
                    anime({
...
function applyFilters() {
    ...
    if (startDate) {
        ...
        filteredGames = filteredGames.filter(game => new Date(game.start_time) >= start);
    }
    
    if (endDate) {
        const end = new Date(endDate);
        filteredGames = filteredGames.filter(game => new Date(game.end_time) <= end);
    }
