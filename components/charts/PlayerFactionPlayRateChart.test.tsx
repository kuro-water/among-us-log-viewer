...
describe("PlayerFactionPlayRateChart responsive settings", () => {
  it("includes responsive rules to reduce label font sizes on narrower charts", () => {
    ...
    render(<PlayerFactionPlayRateChart options={{ games: [g] }} />);
    ...
    // Check responsive dataLabels adjustments exist
    expect(
      rule720.chartOptions.plotOptions?.pie?.dataLabels?.[0]?.style?.fontSize
    ).toBe("0.65em");
    // small-screen rule hides the name label
    expect(
